...
function kakin(){
  ...
  tousigaku += 1000;
  credit += 46;
  document.getElementById("tousi").innerHTML = tousigaku;
...
function stop(count){
  if(nowStatus){
    ...
    if(!carr[0].f && !carr[1].f && !carr[2].f){
      ...
      //
      setTimeout(function() {
        if (go777 == true && carr[0].value == 7 && carr[1].value == 7 && carr[2].value == 7) {
            ...
            //ペカランプ初期化　もう一度betさせる
            betOk = false;
            document.getElementById("credit").innerHTML = credit += 192;
            document.getElementById("pay").innerHTML = 192;
            removeImage();
            go777 = false;
            push0 = true;
            push1 = true;
          ...
          betOk = false;
          document.getElementById('baba').currentTime = 0; 
